...
router.get('/findnine', async (req, res) => {
    res.json(await Product.findnine(req.query));
});

// 推薦圖片列表
router.get('/findsix', async (req, res) => {
    res.json(await Product.findsix(req.query));
});

//為啥不行的列表
router.get('/getlistdata', async (req, res) => {
    res.json(await Product.getListData(req.query));  
});

// 讀取sid>32區間
router.get('/findsome',async (req, res) => {
    res.json(await Product.findSome());
} );
